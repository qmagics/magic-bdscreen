...
// 画横尺
export const drawHorizontalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: { x: number, width: number }, options: { scale: number, width: number, height: number, canvasConfigs: CanvasConfig }) => {
...
// 画竖尺 
export const drawVerticalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: { y: number, height: number }, options: { scale: number, width: number, height: number, canvasConfigs: CanvasConfig }) => {
    const { scale, width, height, canvasConfigs } = options
    const { bgColor, fontColor, shadowColor, ratio, longfgColor, shortfgColor } = canvasConfigs
